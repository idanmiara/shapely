...
def transform(
    ...
    include_z : bool or int, default False
        ...
        If -1 (available only with ``rebuild=True``), include the
            third dimension in the coordinates array that is passed to the
    ...
    single_geom = isinstance(geometry, shapely.Geometry)
    if include_z == -1:
    ...
    if np.isscalar(include_z):
        include_z = [include_z] * len(geometry)
    return np.array(
        [
            transform_wrapper(
                g,
            ...
            for g, z in zip(geometry, include_z)
        ]
    )
