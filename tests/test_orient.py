...
class OrientTestCase(unittest.TestCase):
    ...
    def test_vectorized(self):
        ...
        polygon_with_holes_mixed = Polygon(ring_ccw, [ring_cw, ring_ccw2, ring_cw2, ring_ccw])
        polygon_with_holes_ccw = Polygon(ring_ccw, [ring_cw, ring_ccw2.reverse(), ring_cw2, ring_ccw.reverse()])

        assert_array_equal(orient(polygon_with_holes_ccw, 1), polygon_with_holes_ccw)
        assert_array_equal(orient(polygon_with_holes_ccw, -1), polygon_with_holes_ccw.reverse())
        assert_array_equal(orient(polygon_with_holes_mixed, 1), polygon_with_holes_ccw)
        assert_array_equal(orient(polygon_with_holes_mixed, -1), polygon_with_holes_ccw.reverse())

        # vectorized with scalar sign parameter
        assert_array_equal(orient([ring_ccw, ring_cw, polygon_cw, polygon_with_holes_mixed], 1),
        ...
        assert_array_equal(orient([ring_ccw, ring_cw, polygon_cw, polygon_with_holes_mixed, polygon_cw], [1, -1, 1, -1, -1]),
                           [ring_ccw, ring_cw, polygon_cw.reverse(), polygon_with_holes_ccw.reverse(), polygon_cw])
